"""Utilities for maintaining the AI Trader portfolio.
...
PORTFOLIO_CSV = DATA_DIR / "ai_trader_portfolio_update.csv"
TRADE_LOG_CSV = DATA_DIR / "ai_trader_trade_log.csv"
...
def log_manual_buy(
    ...
    if interactive:
        ...
        if check == "1":
            print("Returning...")
            return cash, ai_portfolio

    if not isinstance(ai_portfolio, pd.DataFrame) or ai_portfolio.empty:
        ai_portfolio = pd.DataFrame(
    ...
    rows = ai_portfolio.loc[ai_portfolio["ticker"].str.upper() == ticker.upper()]
    if rows.empty:
        if ai_portfolio.empty:
            ai_portfolio = pd.DataFrame([{
                "ticker": ticker,
                "shares": float(shares),
        ...
        idx = rows.index[0]
        cur_shares = float(ai_portfolio.at[idx, "shares"])
        cur_cost = float(ai_portfolio.at[idx, "cost_basis"])
        new_shares = cur_shares + float(shares)
        new_cost = cur_cost + float(cost_amt)
        ai_portfolio.at[idx, "shares"] = new_shares
        ai_portfolio.at[idx, "cost_basis"] = new_cost
        ai_portfolio.at[idx, "buy_price"] = new_cost / new_shares if new_shares else 0.0
        ai_portfolio.at[idx, "stop_loss"] = float(stoploss)
...
If this is a mistake, enter 1, or hit Enter."""
    ...
    if ticker not in ai_portfolio["ticker"].values:
        print(f"Manual sell for {ticker} failed: ticker not in portfolio.")
        return cash, ai_portfolio

    ticker_row = ai_portfolio[ai_portfolio["ticker"] == ticker]
    ...
    if total_shares == shares_sold:
        ai_portfolio = ai_portfolio[ai_portfolio["ticker"] != ticker]
    else:
        row_index = ticker_row.index[0]
        ai_portfolio.at[row_index, "shares"] = total_shares - shares_sold
        ai_portfolio.at[row_index, "cost_basis"] = (
            ai_portfolio.at[row_index, "shares"] * ai_portfolio.at[row_index, "buy_price"]
        )
...
def daily_results(ai_portfolio: pd.DataFrame, cash: float) -> None:
    """Print daily price updates and performance metrics (incl. CAPM)."""
    portfolio_dict: list[dict[Any, Any]] = ai_portfolio.to_dict(orient="records")
    today = check_weekend()
    ...
    logger.info("Reading CSV file: %s", PORTFOLIO_CSV)
    ai_trader_df = pd.read_csv(PORTFOLIO_CSV)
    logger.info("Successfully read CSV file: %s", PORTFOLIO_CSV)

    # Use only TOTAL rows, sorted by date
    totals = ai_trader_df[ai_trader_df["Ticker"] == "TOTAL"].copy()
    if totals.empty:
        print("\n" + "=" * 64)
...
def main(data_dir: Path | None = None) -> None:
    ...
    ai_portfolio, cash = load_latest_portfolio_state()
    ai_portfolio, cash = process_portfolio(ai_portfolio, cash)
    daily_results(ai_portfolio, cash)
