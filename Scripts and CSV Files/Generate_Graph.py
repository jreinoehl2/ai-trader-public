...
PORTFOLIO_CSV = f"{DATA_DIR}/ai_trader_portfolio_update.csv"
...
def load_portfolio_totals() -> pd.DataFrame:
    ...
    ai_trader_totals = ai_trader_df[ai_trader_df["Ticker"] == "TOTAL"].copy()
    ai_trader_totals["Date"] = pd.to_datetime(ai_trader_totals["Date"])
    ai_trader_totals["Total Equity"] = pd.to_numeric(
        ai_trader_totals["Total Equity"], errors="coerce"
    )
    ...
    out = pd.concat([baseline_row, ai_trader_totals], ignore_index=True).sort_values("Date")
...
def main() -> dict:
    ...
    ai_trader_totals = load_portfolio_totals()

    start_date = pd.Timestamp("2025-06-27")
    end_date = ai_trader_totals["Date"].max()
    sp500 = download_sp500(start_date, end_date)

    # metrics
    largest_start, largest_end, largest_gain = find_largest_gain(ai_trader_totals)
    dd_date, dd_value, dd_pct = compute_drawdown(ai_trader_totals)
    ...
    plt.plot(
        ai_trader_totals["Date"],
        ai_trader_totals["Total Equity"],
    ...
    largest_peak_value = float(
        ai_trader_totals.loc[ai_trader_totals["Date"] == largest_end, "Total Equity"].iloc[0]
    )
    plt.text(
        largest_end,
    ...
    # annotate final P/Ls
    final_date = ai_trader_totals["Date"].iloc[-1]
    final_chatgpt = float(ai_trader_totals["Total Equity"].iloc[-1])
